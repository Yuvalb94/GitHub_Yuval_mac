...
if __name__ == "__main__":
    ...
    minute_counter = 1
    hour_loop_start_time = datetime.datetime.now().strftime("%H_%M_%S")
    print(hour_loop_start_time)
    scale = []
    for i in range(10):
        scale.append([datetime.datetime.now().strftime("%H_%M_%S"), i])

    print([item[0] for item in scale])
    # while True: 
